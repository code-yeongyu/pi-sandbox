...
export function toAgentContextBlock(effectivePolicy: EffectivePolicy, capability: BackendCapability): string {
	...
	const fileClasses = [
		capability.fileRead ? "project root read:available" : "project root read:omitted",
		capability.fileWrite ? "project root write:available" : "project root write:omitted",
		"home secrets:denied",
	];
	return [
