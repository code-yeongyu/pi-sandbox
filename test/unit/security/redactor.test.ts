...
function emptyState(): RedactionState {
	return { buffer: Buffer.alloc(0), redactedSpans: [] };
}
...
describe("redactor", () => {
	it("#given split chunk secret #when chunks are redacted #then final output redacts secret", () => {
		...
		const second = redactChunk(
			Buffer.from("secret suffix"),
			{ buffer: first.carryover, redactedSpans: [] },
			patterns,
		);
		const output = Buffer.concat([
			first.output,
			second.output,
			flushCarryover({ buffer: second.carryover, redactedSpans: [] }, patterns),
		]);
	...
	it("#given multiple secret patterns #when chunk is redacted #then all patterns are replaced", () => {
		...
		const text = Buffer.concat([
			chunk.output,
			flushCarryover({ buffer: chunk.carryover, redactedSpans: [] }, patterns),
		]).toString("utf8");
	...
	it("#given concurrent states #when chunks differ #then carryover is isolated", () => {
		...
		const completeLeft = redactChunk(Buffer.from("secret"), { buffer: left.carryover, redactedSpans: [] }, patterns);
		expect(
			Buffer.concat([
				completeLeft.output,
				flushCarryover({ buffer: completeLeft.carryover, redactedSpans: [] }, patterns),
			]).toString("utf8"),
		).toContain("[REDACTED:API_TOKEN]");
		expect(flushCarryover({ buffer: right.carryover, redactedSpans: [] }, patterns).toString("utf8")).toBe(
			"ordinary",
		);
	});
